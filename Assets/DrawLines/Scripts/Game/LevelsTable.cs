...
public class LevelsTable : MonoBehaviour
{
		...
		public void OnEnable ()
		{
				try {
						...
						foreach (GameObject gameOb in borders) {
							gameOb.GetComponent<Image> ().color = Color.red;//Mission.wantedMission.missionColor;
						}
				} catch (Exception ex) {
		...
		//LevelsManager levelsManagerComponent = Mission.wantedMission.levelsManagerComponent;
				LevelsManager levelsManagerComponent = GetComponent<LevelsManager> ();
				...
				ID = UnityEngine.Random.Range (1, levelsManagerComponent.levels.Count + 1);
		...
		private void SettingUpLevel (TableLevel tableLevel, int ID)
		{
				...
				tempMissionData = DataManager.FindMissionDataById (1, DataManager.filterdMissionsData);
				...
				tableLevel.transform.Find ("LevelTitle").GetComponent<Text> ().text = "Go"; //ID.ToString ();
